...
## Fourth-pass findings (parity sweep after the discography fix, 2026-07-25)

A sweep of ~15 artists on the 20260725 dataset against live `api.lidarr.audio` confirmed album counts now align (Radiohead 577=577, Miles Davis 749=749, Taylor Swift 410=410, Prince/MJ exact). Remaining "only-ours" albums are 0 foreign-lead and are newer release groups from our fresher export, not bugs. Ratings and image presence match. Two items surfaced:

39. **Credit member order followed dump row order, not MusicBrainz position** (functional, resolved 2026-07-25 with tests).
    `readArtistCreditName` appended credit members in the order the dump delivered them, and `artistIDs[0]` is taken as the album's primary artist (its `artistid` and the discography it lands in). The dump does not deliver credit rows in position order, so a classical work could be led by a performer instead of the composer: Bach's `Matthäus-Passion` and `Mass in B-Minor` were attributed to Peter Schreier / a performer in our data, while both MusicBrainz position 0 and upstream `artistid` are Bach. Fix: keep each credit's members ordered by the `position` column (`addCreditMember` insertion sort), so `artistIDs[0]` is the true lead. This also makes the album payload's `artists[]` follow MusicBrainz position order on every multi-artist credit, not just where the lead changed. Guarded by `TestCreditLeadFollowsPositionNotRowOrder`; `TestCollaborationBelongsToLeaderOnly` now also builds its credit rows out of position order.

35. **Genre tie-break - investigated, wontfix (2026-07-25).** Confirmed both paths sort by MusicBrainz vote count descending, matching upstream; only the order among equal-count genres differs, and ours is alphabetical. Upstream's tie order (Radiohead count-1 group: idm, chamber pop, indietronica, post-grunge, britpop, electronic rock, ambient pop, crossover prog) matches none of the available keys - not name, not genre gid, not MB response order (which is alphabetical), not count. It is upstream's database physical order, arbitrary and not reproducible from the export. Chasing it would be guessing. The genre set matches exactly; keep the deterministic alphabetical tie-break.
